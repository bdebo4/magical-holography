"""WH10 hardware-analysis pipeline: separate boundary/bulk circuit families.

Canonical source: ``WH10-D-C024``, reading from the Phase-1 manifest instead
of live job-status CSVs. Unlike SC8, WH10's 81 boundary and 9 bulk settings
are genuinely different executed circuits (see
``construction_wh10.build_boundary_and_bulk_base``): boundary tomography
(4-qubit, ``boundary_ind=[3,4,8,9]``) is measured on the pre-recovery
encoded state; bulk tomography (2-qubit, ``bulk_indices=[0,5]``) is measured
after the Clifford recovery. Both go through the same "reduce the full
10-qubit circuit-level counts to the tomography register" remap step as
SC8's boundary settings.
"""
...
BULK_BASIS_SET = list(product(["X", "Y", "Z"], repeat=2))
...
def analyze_condition(
    ...
    *,
    ...
    if len(bulk_by_pauli) != 9:
        raise ValueError(f"Condition {condition_id}: expected 9 bulk settings, found {len(bulk_by_pauli)}.")

    def full_counts_for(circuit_name: str) -> tuple[dict[str, float], int]:
        jobs = jobs_by_circuit.get(circuit_name, [])
        width = len(jobs[0].mapping) if jobs else 10
    ...
    measure_bulk: dict = {}
    shots_b = shots_k = 0
    per_circuit_full_counts: dict[str, dict[str, float]] = {}
    ...
    if bootstrap_repeats > 0:
        ...
        for r in range(bootstrap_repeats):
            ...
            _, bulk_boot[r] = state_reconstruction_from_pauli_expectations(mk, [0, 1], use_mlm=True)
